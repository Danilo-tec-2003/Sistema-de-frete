 ...
 *
 * TRANSAÇÕES JDBC:
 *  Toda operação que toca mais de uma tabela usa conn.setAutoCommit(false)
 *  com rollback em caso de falha. Nunca se expõe SQLException ao controlador.
 */
public class FreteBO {
     ...
     *  2. Datas consistentes
     *  3. Valores monetários positivos
     ...
     *  8. Cálculo automático do ICMS se apenas a alíquota for informada
     *
     ...
     *
     * Transação JDBC:
     *  1. Atualiza frete (status S, data_saida)
     *  2. Atualiza veiculo (status V = Em Viagem)
     *  3. Registra ocorrência P (Saída do Pátio)
     */
     ...
     *
     * Transação JDBC:
     *  1. Atualiza frete (status R, data_entrega)
     *  2. Atualiza veiculo (status D = Disponível)
     *  3. Registra ocorrência E (Entrega Realizada) — exige recebedor
     */
     ...
     *
     * Transação JDBC: frete + veiculo + ocorrência.
     */
    ...
    /**
     * Cancela o frete (qualquer estado aberto).
     * Libera o veículo se estava Em Viagem.
     */
    ...
    /* =========================================================
       OCORRÊNCIAS AVULSAS (Avaria, Extravio, Outros, Em Rota)
       ========================================================= */
    ...
    private void validarCamposObrigatorios(Frete f) throws CadastroException {
        ...
        if (f.getIdRemetente() == f.getIdDestinatario())
            throw new CadastroException("Remetente e Destinatário não podem ser o mesmo cliente.");
        ...
        if (f.getUfOrigem() != null && f.getUfOrigem().length() != 2)
            throw new CadastroException("UF de Origem inválida — informe 2 letras.");
        if (f.getUfDestino() != null && f.getUfDestino().length() != 2)
            throw new CadastroException("UF de Destino inválida — informe 2 letras.");
    ...
    private void validarDatas(Frete f) throws CadastroException {
        ...
        if (f.getDataPrevEntrega() != null && f.getDataPrevEntrega().isBefore(hoje))
    ...
    private void validarValores(Frete f) throws CadastroException {
        if (f.getPesoKg() != null && f.getPesoKg().compareTo(BigDecimal.ZERO) < 0)
            throw new CadastroException("O Peso da Carga não pode ser negativo.");
        if (f.getVolumes() != null && f.getVolumes() < 0)
            throw new CadastroException("O número de Volumes não pode ser negativo.");
        ...
        if (f.getAliquotaIcms() != null
                && f.getAliquotaIcms().compareTo(new BigDecimal("100")) > 0)
        ...
        try {
            ...
            if (m.getCnhValidade() != null && m.getCnhValidade().isBefore(LocalDate.now()))
                throw new FreteException(
                    ...
                    + m.getCnhValidade() + ". Regularize antes de emitir o frete.");
    ...
    private void validarOcorrencia(OcorrenciaFrete oc) throws CadastroException {
        ...
        if (tipo.isExigeDescricao()) {
            if (oc.getDescricao() == null || oc.getDescricao().trim().isEmpty())
                throw new CadastroException("Descrição é obrigatória para ocorrência do tipo '"
                    + tipo.getDescricao() + "'.");
        }
    }
